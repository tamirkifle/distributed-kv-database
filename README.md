...
counters, p50/p95/**p99** latency, and repair activity.
...
watch the p99-latency and quorum-failure panels register the blip and recover.

![Grafana p99 under failure](docs/images/grafana-p99-under-failure.png)

> The screenshot above is a manual capture step (requires a live cluster + Grafana). If it
> is not yet present, run the monitoring overlay and the `kill -9` demo above and save the
> p99 panel to `docs/images/grafana-p99-under-failure.png`.
...
- **Monotonic-clock latency measurement.** Quorum read/write latency is timed with
  ...
  exactly this reason.
- **Per-request deadlines on the quorum fan-out.** `LeaderlessKVCluster` fans out to replicas
  synchronously with no per-replica timeout, so a dead/slow replica head-of-line-blocks the request
  threads of its ring neighbors. A real deployment needs per-request deadlines and a concurrent
  fan-out (hedged requests).
- **No snapshotting on the Raft log.** The Raft WAL grows unbounded (acknowledged in the Phase 3
  design); log compaction / snapshot install is the natural next step.
